...
The schematic block diagram below illustrates the relational cardinality across the database workspace:
...
The structured DDL script below creates the tables, setups up primary/foreign key relations, and optimizes the database structure:
...
This centralized routine manages the appointment booking process, ensuring clean data insertion from external calling interfaces:
...
BEGIN
    ...
    VALUES (p_id, d_id, a_date, a_time, 'Scheduled');
END //
DELIMITER ;
10. Trigger Implementation: GenerateBill
This automated trigger keeps billing records in sync. As soon as a new appointment is scheduled, it instantly adds an itemized line item to the billing ledger with a standard checkup fee of $50.00:
...
BEGIN
    ...
    VALUES (NEW.appointment_id, 50.00, 'Unpaid');
END //
DELIMITER ;
...
These diagnostic scripts extract data layers across relationships:
...
SELECT a.appointment_id, p.name AS Patient, d.name AS Doctor, a.appointment_date, b.total_amount
FROM Appointments a
